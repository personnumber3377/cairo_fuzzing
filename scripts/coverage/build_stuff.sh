...
RUN_SECONDS=3000                          # how many seconds to run each fuzzer to collect profiles
...
# These here originally also included "gio-2.0"

DEPS="gmodule-2.0 glib-2.0 gobject-2.0 freetype2 cairo cairo-gobject"
...
for srcf in $fuzzer_sources; do
  ...
  # compile .o
  $CC $BUILD_CFLAGS -DCOVERAGE_BUILD=1 -c "$srcf" -o "$WORK/${fuzzer_name}.o"
  ...
  $CXX $CXXFLAGS -DCOVERAGE_BUILD=1 \
